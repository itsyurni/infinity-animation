...
it.fn.css = function(param,value){
    if(is.object(param) && is.undefined(value)){
		...
		this.elem[0].style[param] = value;
    }else if(is.string(param) && is.undefined(value)){
		return _getComputedStyle(this.elem[0],param);
	}
    return this;
};
...
it.fn.animate = function(prop = {},generalOption = {}){
    ...
    */
    this.chain(function(Queue){
        ...
        */
        var reverseData;
        if(is.undefined(this["reverseData"])){
...
Tween.parsing = {
	computedStyle : function(elem,prop){
		return _getComputedStyle(elem,prop)
	},
	to : function(from,to){
 ...
 Tween.fn.tweens = {
	_default : {
    	...
    	set: function( tween ) {
			...
			tween.elem.style[tween.prop] = tween.step + tween.unit;		
    	}
	},
};
